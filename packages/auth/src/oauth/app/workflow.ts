...
import { makeSessionSigningCodec } from "../../sessions/crypto";
...
import { retainGrantTokens, type PreviousGrantTokens } from "../grantTokens";
...
>(
  ...
  const Service = Context.Service<
    { readonly app: Id; readonly kind: "workflow" },
    Workflow<Session>
  >()(`effect-auth/OAuthApp/${id}/Workflow`);
  ...
  const layer = <E, R>(input: Options<E, R> & { readonly callbackUrl: string }) => {
    const config = { ...input };
    ...
    return Layer.effectContext(
      Effect.gen(function* () {
        ...
        const safe = <A, E2, R2>(effect: Effect.Effect<A, E2, R2>) =>
          effect.pipe(
            Effect.catchCause((cause): Effect.Effect<never, E2 | OAuthUnavailable> => {
              if (Cause.hasInterrupts(cause)) return Effect.failCause(cause);
              if (!Cause.hasDies(cause)) return Effect.failCause(cause);

              return reportAuthFailure("oauth-app", cause).pipe(
                Effect.andThen(Effect.fail(OAuthUnavailable.make({}))),
              );
            }),
          );
        ...
        const makeGrant = Effect.fn("OAuthApp.makeGrant")(function* (
          ...
          const retained = yield* retainGrantTokens(configuration.profile, grant, previous);

          const refreshUntil =
            retained.material.refreshToken === undefined
              ? undefined
              : Math.min(
                  retained.refreshExpiresAtMillis ?? Infinity,
                  time +
          ...
          const context = yield* snapshotOAuth(OAuthConnectedTokenContext, {
            ...
            metadata: {
              ...
              ...(retained.refreshExpiresAtMillis === undefined
                ? {}
                : { refreshExpiresAtMillis: retained.refreshExpiresAtMillis }),
          ...
          return GrantRecord.make({
            ...
            status: "Active",
            context,
            sealed: yield* tokens.seal({ context, material: retained.material }),
          });
        });
        ...
        return Context.make(Service, { begin, complete, withAccessToken, disconnect }).pipe(
