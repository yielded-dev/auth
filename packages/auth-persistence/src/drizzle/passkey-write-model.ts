...
> {
  readonly credential: {
    ...
    readonly encodeInsert: (input: PasskeyCredentialInsert<N>) => InferInsertModel<C>;
  ...
  readonly authority: {
    readonly encodeInsert: (input: PasskeyCredentialInsert<N>) => InferInsertModel<F>;
    readonly activeStatus: unknown;
    readonly removedStatus: unknown;
  };
  readonly credentialOwnership: {
    readonly encodeInsert: (input: PasskeyCredentialInsert<N>) => InferInsertModel<O>;
  ...
  readonly policy: {
    ...
    readonly management: (row: Readonly<Partial<InferSelectModel<S>>>) => PasskeyManagementPolicy;
    readonly requirement: (
      row: Readonly<Partial<InferSelectModel<S>>>,
      action: PasskeyActionAuthorization["challenge"]["action"],
    ) => Effect.Effect<typeof PasskeyRequirement.Type, PersistenceMappingError>;
    readonly metadata: (subjectId: N) => SQL;
    readonly action: (subjectId: N, authorization: PasskeyActionAuthorization) => SQL;
    ...
    readonly remainingSignIn: (
      ...
      row: Readonly<Partial<InferSelectModel<S>>>,
    ) => Effect.Effect<SQL, PersistenceMappingError>;
  };
}
...
export interface PasskeyInvalidationMutation<N> {
...
export const passkeyInvalidationMutation = <T extends Table, N>(input: {
  ...
  readonly postcondition: (input: PasskeyInvalidationInput<N>) => SQL;
}): PasskeyInvalidationMutation<N> => input;
...
> extends PasskeyPersistenceMapping<
  ...
  N
> {
  readonly write: PasskeyWriteTables<S, C, F, O, H, N>;
  ...
  readonly invalidation: {
    ...
    readonly mutations: ReadonlyArray<PasskeyInvalidationMutation<N>>;
    readonly postcondition: (input: PasskeyInvalidationInput<N>) => SQL;
  };
}
...
export interface PasskeyRegistrationWriter<R> {
  ...
  readonly completeRegistration: <A>(
    ...
    prepare: PreparePasskeyCommit<PasskeyRegistrationResult | { readonly _tag: "Rejected" }, A>,
  ) => Effect.Effect<PreparedCommit<A>, PasskeyUnavailable>;
}
...
> {
  ...
  readonly handle: PasskeyHandleReservationTable<H> & {
  ...
  readonly registration: {
    ...
    readonly describe: (registration: R) => { readonly name: string; readonly displayName: string };
    readonly eligible: (registration: R) => SQL;
    ...
    readonly finalEligibility: (input: { readonly registration: R; readonly subjectId: N }) => SQL;
    readonly subject: (input: {
      readonly registration: R;
      readonly ceremony: PasskeyCeremony;
      readonly marker: string;
    }) => { readonly subjectId: N; readonly values: InferInsertModel<S> };
    readonly activeStatus: unknown;
  };
