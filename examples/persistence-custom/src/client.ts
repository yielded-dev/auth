...
// UI workflows share the application registry and can finish across sign-in/out.
export const makeClient = (
  ...
  const AppClient = Client.make(AuthApi, options);
  const auth = AuthAtom.make(AppClient, { runtime: factory });
    ...
    Effect.fn("Customers.sendVerification")(function* (input: string, get: Atom.FnContext) {
      ...
      if (current !== null && current.email === email && now < current.expiresAtMillis) {
        ...
        get.set(verification, {
          ...
          resendAtMillis: (yield* nowMillis) + emailProofPolicy.abuse.resendCooldownMillis,
        });
  ...
  const createAccount = runtime.fn<{
    ...
    Effect.fn("Customers.createAccount")(function* (input, get) {
      ...
      const result = yield* get.setResult(auth.register, {
        requestId: yield* id,
        email: input.email,
        newPassword: input.password,
        registration: { displayName: input.displayName, username: input.username },
      });
      ...
      const signedIn = yield* get.setResult(auth.passwordSignIn, {
        login: input.email,
  ...
  const signIn = runtime.fn<{ readonly login: string; readonly password: string }>()(
    Effect.fn("Customers.signIn")(function* (input, get) {
      get.set(notice, null);
      get.set(verification, null);
      get.set(recovery, null);

      return yield* get.setResult(auth.passwordSignIn, input);
    }),
  );
  ...
  const requestReset = runtime.fn<string>()(
    Effect.fn("Customers.requestReset")(function* (input, get) {
      ...
      get.set(recovery, {
        ...
        flowId,
        commandId: yield* id,
        expiresAtMillis: now + emailProofPolicy.lifetimeMillis,
        resendAtMillis: (yield* nowMillis) + emailProofPolicy.abuse.resendCooldownMillis,
      });
    }),
  );
